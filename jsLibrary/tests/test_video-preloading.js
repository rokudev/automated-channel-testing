...
describe('test_video-preloading', () => {
    before(() => {
    ...
    it('should launch the channel', async function() { 
        this.timeout(25000);
        await library.sideLoad("../channels/video_preloading.zip", "rokudev", "aaaa");
    ...
    it('Verify endcard after first video', async function() { 
        this.timeout(80000);
        const res = await library.verifyIsScreenLoaded({'elementData': [{'using': 'tag', 'value': 'EndcardView'}]}, 15, 5);
        expect(res).equal(true);
    });

    
    it('Verify is playback started after 10 seconds', async function() { 
        this.timeout(50000);
        const res = await library.verifyIsPlaybackStarted(3, 5);
        expect(res).equal(true);
    });
